	...
	if(is_multisite()){
		...
		if (!in_array('htmlemail/htmlemail.php', apply_filters('active_plugins', get_option('active_plugins')))) {
	        function sample_admin_notice__error()
	        {
	            ...
	            $message = __('TODO: Please, install HTML Email Plugin before activating plugin.');

	            printf('<div class="%1$s"><p>%2$s</p></div>', esc_attr($class), esc_html($message) . ' <a href="http://drop.frontporchsolutions.com/b5LmXd" target="_blank">Get It Here</a>');
	        }
